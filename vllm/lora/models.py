...
class LoRAModel:
    ...
    def __init__(
        ...
        self.rank = rank
        # key表示模型中某个具体层的名称/路径（如 "transformer.layer.0.attention"）
        # value存储该层对应的LoRA权重参数（即低秩矩阵A和B等）
    ...
    # 将输入的 LoRA 权重张量（如 lora_A 和 lora_B 矩阵）解析并组织到模型的各个层中，
    # 生成一个包含所有 LoRA 参数的 LoRAModel 对象。
    @classmethod
    ...
    # 该方法通过读取本地目录中的 LoRA 检查点文件（配置、权重、嵌入扩展等），
    # 将这些数据整合后调用 from_lora_tensors 方法，最终生成一个 LoRAModel 对象。
    # 适用于从磁盘加载已保存的 LoRA 模型。
    @classmethod
